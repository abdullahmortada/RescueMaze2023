...
def detect_colorslog(image_path):
    # Load the image
    ...
    boundaries = {
        'red': ([1, 84, 78], [9, 255, 255]),
        'green': ([43, 60, 50], [64, 255, 255]),
        'yellow': ([25, 60, 99], [55, 255, 255]),
...
def detect_colorspi(image_path):
    # Load the image
    img = cv2.imread(image_path)
    imgHSV = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
...
def main():
    for i in range(7):
        image_path = f'pig{i}.jpg'
        colors = detect_colorspi(image_path)
        print(colors)
